import { expect, Locator, Page } from '@playwright/test'

export class OrdersPage {
  readonly page: Page;
  readonly ordersHeading: Locator;
  readonly ordersTable: Locator;
  readonly orderRows: Locator;
  readonly createOrderButton: Locator;
  readonly searchBox: Locator;
  readonly searchButton: Locator;
  readonly filterDropdown: Locator;
  readonly statusFilter: Locator;
  readonly dateRangeFilter: Locator;
  readonly refreshButton: Locator;
  readonly exportButton: Locator;
  readonly deleteButton: Locator;
  readonly editButton: Locator;
  readonly viewDetailsButton: Locator;
  readonly saveButton: Locator;
  readonly cancelButton: Locator;
  readonly confirmDialog: Locator;
  readonly confirmYesButton: Locator;
  readonly confirmNoButton: Locator;

  constructor(page: Page) {
    this.page = page;
    
    // Navigation & Headers
    this.ordersHeading = page.getByRole('heading', { name: /Orders/i });
    
    // Table elements
    this.ordersTable = page.locator('table');
    this.orderRows = this.ordersTable.locator('tbody tr');
    
    // Buttons
    this.createOrderButton = page.getByRole('button', { name: /create order|new order|add order/i }).first();
    this.searchButton = page.getByRole('button', { name: /search|query/i }).first();
    this.refreshButton = page.getByRole('button', { name: /refresh|reload/i }).first();
    this.exportButton = page.getByRole('button', { name: /export|download/i }).first();
    this.deleteButton = page.getByRole('button', { name: /delete|remove/i }).first();
    this.editButton = page.getByRole('button', { name: /edit|modify/i }).first();
    this.viewDetailsButton = page.getByRole('button', { name: /view|details|expand/i }).first();
    this.saveButton = page.getByRole('button', { name: /save|submit|ok/i }).first();
    this.cancelButton = page.getByRole('button', { name: /cancel|close/i }).first();
    
    // Filters & Search
    this.searchBox = page.getByPlaceholder('Search').or(page.getByRole('searchbox', { name: 'Search' })).first();
    this.filterDropdown = page.getByRole('combobox', { name: /filter|status/i }).first();
    this.statusFilter = page.getByRole('combobox', { name: /status/i }).first();
    this.dateRangeFilter = page.getByRole('textbox', { name: /date|from|to/i }).first();
    
    // Dialog
    this.confirmDialog = page.locator('[role="dialog"]').first();
    this.confirmYesButton = page.getByRole('button', { name: /yes|confirm|ok|proceed/i }).first();
    this.confirmNoButton = page.getByRole('button', { name: /no|cancel|decline/i }).first();
  }

  async navigateToOrders() {
    // First try direct navigation
    await this.page.goto('/sales/orders', { waitUntil: 'load' }).catch(async () => {
      // If goto fails, try using the menu
      const salesMenu = this.page.getByRole('link', { name: 'Sales' });
      const ordersLink = this.page.getByRole('link', { name: /^Orders$/ });
      
      if (await salesMenu.count() > 0) {
        await salesMenu.click();
        await this.page.waitForTimeout(500);
      }
      
      if (await ordersLink.count() > 0) {
        await ordersLink.click();
        await this.page.waitForTimeout(1000);
      }
    });
    
    // Wait for table to be visible as confirmation page loaded
    await this.ordersTable.waitFor({ state: 'visible', timeout: 10000 }).catch(() => {});
  }

  async searchOrders(searchTerm: string) {
    if (await this.searchBox.count() > 0) {
      await this.searchBox.fill(searchTerm);
      if (await this.searchButton.count() > 0) {
        await this.searchButton.click();
      } else {
        await this.searchBox.press('Enter');
      }
      await this.page.waitForTimeout(500);
    }
  }

  async filterByStatus(status: string) {
    if (await this.statusFilter.count() > 0) {
      await this.statusFilter.click();
      const option = this.page.getByRole('option', { name: new RegExp(status, 'i') });
      if (await option.count() > 0) {
        await option.click();
      }
      await this.page.waitForTimeout(500);
    }
  }

  async openOrderByIndex(index: number) {
    const count = await this.orderRows.count();
    if (count <= index) throw new Error(`Order row index ${index} out of bounds (count: ${count})`);
    const row = this.orderRows.nth(index);
    await row.click();
    await this.page.waitForTimeout(800);
  }

  async openOrderByOrderId(orderId: string) {
    const orderCell = this.page.locator('table').locator(`text="${orderId}"`).first();
    if (await orderCell.count() > 0) {
      const row = orderCell.locator('xpath=ancestor::tr');
      await row.click();
      await this.page.waitForTimeout(800);
    }
  }

  async createNewOrder() {
    if (await this.createOrderButton.count() > 0) {
      await this.createOrderButton.click();
      await this.page.waitForTimeout(1000);
    }
  }

  async fillOrderForm(data: { customer?: string; amount?: string; items?: string; notes?: string }) {
    // Fill customer field
    if (data.customer) {
      const customerInput = this.page.getByLabel(/customer|customer name/i).first();
      if (await customerInput.count() > 0) {
        await customerInput.fill(data.customer);
        const suggestion = this.page.getByRole('option', { name: new RegExp(data.customer, 'i') }).first();
        if (await suggestion.count() > 0) await suggestion.click();
      }
    }

    // Fill amount field
    if (data.amount) {
      const amountInput = this.page.getByLabel(/amount|total|price/i).first();
      if (await amountInput.count() > 0) {
        await amountInput.fill(data.amount);
      }
    }

    // Fill items field (sketch)
    if (data.items) {
      const itemsInput = this.page.getByLabel(/items|products|line items/i).first();
      if (await itemsInput.count() > 0) {
        await itemsInput.fill(data.items);
      }
    }

    // Fill notes field
    if (data.notes) {
      const notesInput = this.page.getByLabel(/notes|remarks|description/i).first();
      if (await notesInput.count() > 0) {
        await notesInput.fill(data.notes);
      }
    }
  }

  async saveOrder() {
    if (await this.saveButton.count() > 0) {
      await this.saveButton.click();
      await this.page.waitForTimeout(1000);
    }
  }

  async deleteOrderByIndex(index: number) {
    const row = this.orderRows.nth(index);
    const deleteBtn = row.locator('button:has-text("Delete")').or(row.locator('[aria-label*="delete" i]')).first();
    if (await deleteBtn.count() > 0) {
      await deleteBtn.click();
      // Confirm deletion
      if (await this.confirmYesButton.count() > 0) {
        await this.confirmYesButton.click();
      }
      await this.page.waitForTimeout(800);
    }
  }

  async refreshTable() {
    if (await this.refreshButton.count() > 0) {
      await this.refreshButton.click();
      await this.page.waitForTimeout(1000);
    }
  }

  async exportOrders(format: string = 'CSV') {
    if (await this.exportButton.count() > 0) {
      await this.exportButton.click();
      const exportOption = this.page.getByRole('option', { name: new RegExp(format, 'i') }).first();
      if (await exportOption.count() > 0) {
        await exportOption.click();
      }
      await this.page.waitForTimeout(1500);
    }
  }

  async getOrderCount(): Promise<number> {
    return await this.orderRows.count();
  }

  async getRowData(rowIndex: number): Promise<string[]> {
    const row = this.orderRows.nth(rowIndex);
    const cells = row.locator('td');
    const count = await cells.count();
    const data: string[] = [];
    for (let i = 0; i < count; i++) {
      const text = await cells.nth(i).textContent();
      data.push(text?.trim() || '');
    }
    return data;
  }

  async editOrderByIndex(index: number, data: { customer?: string; amount?: string; notes?: string }) {
    await this.openOrderByIndex(index);
    await this.fillOrderForm(data);
    await this.saveOrder();
  }
}
